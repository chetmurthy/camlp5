...
val concat_long_ids : Longident.t -> Longident.t -> Longident.t
...
val class_info :
  (MLast.attributes -> Parsetree.attributes) ->
...
val is_module_path : MLast.expr -> bool
...
val sumbranch_ctyp :
  ...
  MLast.ctyp list ->
  MLast.ctyp option ->
  (Parsetree.core_type list, Parsetree.type_kind) Versdep.choice *
  Parsetree.core_type option
val conv_constructor :
  bool ->
  MLast.generic_constructor ->
  string *
  ((Parsetree.core_type list, Parsetree.type_kind) Versdep.choice *
   Parsetree.core_type option) *
  Location.t * Parsetree.attributes
val mktvariant :
  MLast.loc -> MLast.generic_constructor list -> bool -> Parsetree.type_kind
val mktrecord :
  (MLast.loc * string * bool * MLast.ctyp * MLast.attributes) list ->
  bool -> Parsetree.type_kind
val ctyp : MLast.ctyp -> Parsetree.core_type
val meth_list :
  MLast.loc ->
  (string * MLast.ctyp * MLast.attributes) list ->
  bool Ploc.vala -> Parsetree.object_field list
...
val extension_constructor :
  MLast.loc -> MLast.extension_constructor -> Parsetree.extension_constructor
val type_extension :
  MLast.loc -> MLast.type_extension -> Parsetree.type_extension
...
val mkpe :
  MLast.patt * MLast.expr * MLast.attributes -> Parsetree.value_binding
...
val uv_item_attributes : MLast.attributes -> Parsetree.attributes
val uv_alg_attributes : MLast.attributes -> Parsetree.attributes
val conv_attributes : MLast.attributes_no_anti -> Parsetree.attributes
